...
console.log("staus update::::::", req.body);
...
if(!userSessions.sessionID[last_session]){
  ...
  if (req.body.fulfillmentInfo.tag == 'Tpindtmf'){
    ...
    if (hashedValue == 'baade98b1d01cf744d24025779ed20f635654eea7747febee798109b472ce22f' && userSessions.sessionID[last_session].UserName == 'Sai') {
      ...
      message = userSessions.sessionID[last_session].UserName +' Votre nouveau mot de passe est ' + Password_reset + ' ,,,, ,,Pour répéter votre mot de passe, appuyez sur 1 ,,,,, ou ,,,, si vous souhaitez mettre fin à l’appel, appuyez sur 2';
      ...
      // for (let executionCount = 0; executionCount < 3; executionCount++) {
        if (executionCount < 3) {
        message = userSessions.sessionID[last_session].UserName + ' Veuillez réessayer avec un Tpin valide ou un Tpin associé à votre profil';
        a = 5;
        }   
        else {
        message = '';
...
try {
      ...
      message = 'Est-ce que je parle à ' + response.data.value[0].displayName + ', Si c’est le cas, veuillez confirmer en appuyant sur 1, sinon appuyez sur 2 pour entrer à nouveau votre employé I D';
      Azure_AD = "Correct";
    ...
    message = 'Votre brève description est.' + shortdescription + ' .Dans l’affirmative, veuillez dire oui. Si ce n’est pas le cas, veuillez dire Non';
  } else if (req.body.intentInfo.displayName == 'Userlongdescription') {
    description = req.body.text || req.body.transcript;
    message = 'Votre brève description est. ' + description + ' .Dans l’affirmative, veuillez dire oui. Si ce n’est pas le cas, veuillez dire Non';
    ...
    try {
      const response = await axios.request(config);
      message = 'Nous avons créé un billet pour vous et votre numéro de billet est, ' + response.data.result.number + ' ,La brève description de votre billet est, ' + response.data.result.short_description + ' , et la longue description de votre problème est , ' + response.data.result.description + ' , nous travaillerons à résoudre votre problème au plus tôt!, Pour revenir au menu principal, veuillez indiquer le menu principal ou Si vous voulez le soutien d’un agent en direct, veuillez dire, Agent en direct, si vous voulez mettre fin à l’appel, veuillez dire terminer ou vous déconnecter';
    } catch (error) {
      console.error(error);
 ...
 // else if (req.body.intentInfo.displayName == 'IncidentNumber') {
  ...
  //   message = 'Your Incident Number is ' + incidentNumber + ' If so please say Yes, if not please say No'; }
    ...
    message = 'Votre Tpin est ' + join_Tpin + ' Yess or No';
  }
